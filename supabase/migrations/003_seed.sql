-- ============================================================
-- FORMA — Phase 2: Seed data
-- Run this AFTER 002_catalogue.sql
-- ============================================================
...
insert into public.artists (id, handle, name, city, joined, followers, bio, verified, accent) values
  ('a1', 'kestner.studio', 'Lou Kestner', 'Zürich', '2024', 4218, 'Generative type, halftone studies, post-grid systems.', true, '#FF3B1F'),
  ('a2', 'mira.tanaka', 'Mira Tanaka', 'Kyoto', '2023', 8902, 'Architectural drawing, light + concrete + nothing else.', true, '#0E0E0C'),
...
-- -------------------------------------------------
-- Artworks
-- ends_at uses relative intervals from now() so auctions are always "live"
-- -------------------------------------------------
insert into public.artworks (id, title, visual, artist_id, year, dimensions, edition, start_bid, current_bid, bid_count, watcher_count, ends_at, tags, like_count, format) values
  ('w01', 'Halftone Field №14', 'v1', 'a1', 2026, '4096×4096', '1/1', 280, 480, 14, 92, now() + interval '5 hours 12 minutes', ARRAY['halftone','generative','print-ready'], 211, 'PNG + raw .glyph'),
  ('w02', 'Concrete Light, Study II', 'v2', 'a2', 2026, '3000×3000', '1/3', 600, 1240, 22, 188, now() + interval '22 hours', ARRAY['architecture','gradient','photo'], 540, 'TIFF'),
  ('w03', 'Mondrian Reissue', 'v3', 'a3', 2025, '5000×5000', '1/1', 320, 320, 0, 41, now() + interval '2 days', ARRAY['bauhaus','color-blocks','print'], 88, 'SVG + PNG'),
  ('w04', 'Topographic Drift', 'v4', 'a5', 2026, '4500×4500', '1/5', 180, 245, 5, 67, now() + interval '2 hours 30 minutes', ARRAY['line-art','topography'], 134, 'SVG'),
  ('w05', 'A to Z', 'v5', 'a1', 2026, '6000×6000', '1/1', 540, 820, 9, 102, now() + interval '14 hours', ARRAY['typography','poster'], 312, 'AI + PDF'),
  ('w06', 'Sun, Receding', 'v6', 'a5', 2025, '4000×4000', '1/10', 120, 160, 3, 33, now() + interval '2 days 22 hours', ARRAY['gradient','minimal'], 76, 'PNG'),
  ('w07', 'Orbit, Inner', 'v7', 'a2', 2026, '3000×3000', '1/1', 400, 615, 11, 124, now() + interval '9 hours', ARRAY['line-art','geometry'], 198, 'SVG + PNG'),
  ('w08', 'Channel Error 03', 'v8', 'a4', 2026, '3840×2160', '1/1', 700, 1450, 28, 312, now() + interval '1 hour 6 minutes', ARRAY['glitch','1/1'], 822, 'PNG'),
  ('w09', 'Three Triangles', 'v9', 'a3', 2026, '4000×4000', '1/8', 90, 90, 0, 18, now() + interval '4 days', ARRAY['geometry','bauhaus'], 41, 'SVG'),
  ('w10', 'Hot Sun, Cold Sky', 'v10', 'a6', 2025, '2000×2000', '12/25', 60, 60, 0, 9, now() + interval '5 days', ARRAY['riso','print'], 27, 'TIFF + RISO sep.'),
  ('w11', 'Terminal Garden', 'v11', 'a4', 2026, 'TXT/4096w', '1/1', 240, 360, 7, 58, now() + interval '1 day 12 hours', ARRAY['ascii','code-art'], 144, 'TXT + PNG'),
  ('w12', 'Field, Divided', 'v12', 'a6', 2026, '4000×4000', '1/15', 80, 145, 4, 22, now() + interval '2 days 12 hours', ARRAY['riso','color-blocks'], 63, 'TIFF')
on conflict (id) do nothing;
...
insert into public.commissions (id, artist_id, title, slots, taken, price, days, brief) values
  ...
  ('c4', 'a4', 'Glitched 1/1 portrait', 1, 0, 2000, 21, 'Send me one photo. Get back a destroyed, beautiful piece. 1/1, no prints.'),
...
insert into public.feed_posts (id, artist_id, type, posted_at, body, artwork_id, like_count, comment_count) values
  ('p1', 'a4', 'drop', '2h', 'New 1/1 live. Channel_Err_03. 22 minutes of editing for 4 seconds of decision.', 'w08', 211, 18),
